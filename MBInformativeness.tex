...
%Microblog Relevance Hypotheses ?


%Each Microblog document has a source (The author) and text. Moreover they may indicate an intended audience (Specified by \@ tags), a topical context (specified by \#tags) and finally a link to a document by means of a URL.
...
% More specifically, we should have experienced statistically significant differences when measuring at much lower cut-off points if the scope and verbosity hypotheses were true, as the DFR model promotes longer documents. 
...
%  as we should have experienced differences in the DocLength feature if they were true, given that we even employed a retrieval model that promotes longer documents.

%

%\mentalnote{Also slight variations in length, may cause big effects in the ranking.}

%

%Moreover, as the ranked list has been computed by DFR, we can observe how documents at the top of the list are longer in average than those at the lower cut-off points. This does not affect our results as we are interested in measuring the relative space used for each dimension w.r.t. the available space on each document.
